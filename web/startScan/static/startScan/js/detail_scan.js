function get_ips_from_port(port_number, history_id) {
  document.getElementById("detailScanModalLabel").innerHTML =
    "IPs with port " + port_number + " OPEN";
  var ip_badge = "";
  ...
  var endpoint_table = $("#endpoint_results").DataTable({
    ...
    columnDefs: [
      ...
      },
      {
        render: function (data, type, row) {
          ...
          if (row["techs"]) {
            tech_badge =
              `</br>` +
              parse_technology(row["techs"], "primary", (outline = true));
          }
          ...
          return (
            ...
            "<a href='" +
            data +
            `' id="url-${row["id"]}" target='_blank' class='text-primary'>` +
            url +
            "</a>" +
            tech_badge +
            "<br>" +
      ...
      },
      {
        render: function (data, type, row) {
          if (data) {
            return parse_comma_values_into_span(
              data,
              "danger",
              (outline = true)
            );
  ...
  radioGroup.forEach(function (radioButton) {
    radioButton.addEventListener("change", function () {
      if (this.checked) {
        ...
        var col_index = get_datatable_col_index(
          this.value,
          endpoint_datatable_columns
        );
        api.page.len(-1).draw();
    ...
    columnDefs: [
      ...
      { className: "text-center", targets: [2, 4] },
      {
        render: function (data, type, row) {
          ...
          if (row["is_cdn"]) {
            cdn_badge =
              "<span class='m-1 badge  badge-soft-warning'>CDN</span>";
          }
          if (row["is_interesting"]) {
            interesting_badge =
              "<span class='m-1 badge  badge-soft-danger'>Interesting</span>";
          }
          ...
          if (row["http_url"]) {
            if (row["cname"]) {
              return (
                ...
                data +
                `</a><br><span class="text-dark">CNAME<br><span class="text-warning"> ❯ </span>` +
                row["cname"].replace(
                  ",",
                  '<br><span class="text-warning"> ❯ </span>'
                ) +
                `</span>`
              );
            }
      ...
      },
      {
        render: function (data, type, row) {
          ...
          if (data >= 200 && data < 300) {
            return (
              "<span class='badge  badge-soft-success'>" + data + "</span>"
            );
          } else if (data >= 300 && data < 400) {
            return (
              "<span class='badge  badge-soft-warning'>" + data + "</span>"
            );
          } else if (data == 0) {
...
function get_osint_users(scan_id) {
  $.getJSON(
    `/api/queryOsintUsers/?scan_id=${scan_id}&format=json`,
    function (data) {
      $("#osint-users-count").empty();
      for (var val in data["users"]) {
        user = data["users"][val];
        $("#osint-users").append(
          `<span class='badge badge-soft-info  m-1'>${user["author"]}</span>`
        );
      }
      $("#osint-users-count").html(
        `<span class="badge badge-soft-primary">${data["users"].length}</span>`
      );
      $("body").tooltip({ selector: "[data-toggle=tooltip]" });
    }
  ).fail(function () {
...
function get_screenshot(scan_id) {
  ...
  $.getJSON(
    `/api/listSubdomains/?scan_id=${scan_id}&no_page&only_screenshot`,
    function (data) {
      ...
      for (var subdomain in data) {
        ...
        interesting_field = data[subdomain]["is_interesting"]
          ? "interesting"
          : "";
        ...
        link.setAttribute(
          "href",
          "/media/" + data[subdomain]["screenshot_path"]
        );
        link.setAttribute(
          "data-title",
          `<a target='_blank' href='` +
            data[subdomain]["http_url"] +
        ...
        if (
          data[subdomain]["http_status"] >= 200 &&
          data[subdomain]["http_status"] < 300
        ) {
          http_status_badge = "success";
        } else if (
          data[subdomain]["http_status"] >= 300 &&
          data[subdomain]["http_status"] < 400
        ) {
          http_status_badge = "warning";
        }
        page_title = data[subdomain]["page_title"]
          ? htmlEncode(data[subdomain]["page_title"]) + "</br>"
          : "";
        subdomain_link = data[subdomain]["http_url"]
          ? `<a href="${data[subdomain]["http_url"]}" target="_blank">${data[subdomain]["name"]}</a>`
          : `<a href="https://${data[subdomain]["name"]}" target="_blank">${data[subdomain]["name"]}</a>`;
        http_status = data[subdomain]["http_status"]
          ? `<span class="m-1 float-end badge  badge-soft-${http_status_badge}">${data[subdomain]["http_status"]}</span>`
          : "";
        figcaption.innerHTML = data[subdomain]["is_interesting"]
          ? page_title + subdomain_link + interesting_badge + http_status
        ...
        if (
          !$("#http_select_filter").find(
            "option:contains('" + http_status + "')"
          ).length
        ) {
        ...
        for (var tech in technology) {
          ...
          filter_values +=
            "tech_" + tech_name.replace(/ /g, "_").toLowerCase() + " ";
      ...
      if (port_select) {
        ...
        for (var port in port_array) {
          if (
            !$("#ports_select_filter").find(
              "option:contains('" + port_array[port] + "')"
            ).length
          ) {
            var option = document.createElement("option");
      ...
      for (var ip in ip_array) {
        if (
          !$("#ips_select_filter").find(
            "option:contains('" + ip_array[ip] + "')"
          ).length
        ) {
          var option = document.createElement("option");
      ...
      if (service_select) {
        for (var service in service_array) {
          if (
            !$("#services_select_filter").find(
              "option:contains('" + service_array[service] + "')"
            ).length
          ) {
            var option = document.createElement("option");
      ...
      for (var tech in tech_array) {
        if (
          !$("#tech_select_filter").find(
            "option:contains('" + tech_array[tech] + "')"
          ).length
        ) {
          var option = document.createElement("option");
          option.value =
            ".tech_" + tech_array[tech].replace(/ /g, "_").toLowerCase();
      ...
      var gridzyElements = document.querySelectorAll(
          ".gridzySkinBlank[data-gridzySearchField]"
        ),
      ...
      while (pos--) {
        (function (gridzyElement) {
          var searchField = document.querySelector(
            gridzyElement.getAttribute("data-gridzySearchField")
          );
          ...
          function search() {
            ...
            if (searchValue) {
              while (pos--) {
                ...
                itemContent = (
                  child.getAttribute("data-gridzySearchText") || child.innerText
                ).toLowerCase();
              ...
              if (
                gridzyInstance.getOption("filter") !==
                Gridzy.getDefaultOption("filter")
              ) {
      ...
      $(
        "#http_select_filter, #ips_select_filter, #services_select_filter, #ports_select_filter, #tech_select_filter"
      ).on("change", function () {
        values = $(this).val();
        console.log(values);
...
function get_metadata(scan_id) {
  ...
  $.getJSON(
    `/api/queryMetadata/?scan_id=${scan_id}&format=json`,
    function (data) {
      ...
      for (var val in data["metadata"]) {
        ...
        if (doc["doc_name"]) {
          filename = `<a href=${
            doc["url"]
          } target="_blank" class="text-primary">${truncate(
            doc["doc_name"],
            30
          )}</a>`;
        } else {
        ...
        $(`#${rand_id}`).append(
          `<td class="td-content">${filename}</br>${subdomain}</td>`
        );
        if (doc["author"]) {
          $(`#${rand_id}`).append(
            `<td class="td-content text-center">${doc["author"]}</td>`
          );
        } else {
        ...
        if (doc["producer"] || doc["creator"] || doc["os"]) {
          ...
          metadata += doc["os"]
            ? `<br> <span class='badge badge-soft-danger'> OS: ` +
              doc["os"] +
              "</span>"
            : "";
      ...
      $("#metadata-count").html(
        `<span class="badge badge-soft-primary">${data["metadata"].length}</span>`
      );
      $(".bs-tooltip").tooltip();
    }
  );
...
function get_emails(scan_id) {
  var exposed_count = 0;
  $.getJSON(
    `/api/queryEmails/?scan_id=${scan_id}&format=json`,
    function (data) {
      $("#emails-count").empty();
      ...
      for (var val in data["emails"]) {
        ...
        $(`#${rand_id}`).append(
          `<td class="td-content">${email["address"]}</td>`
        );
        if (email["password"]) {
          $(`#${rand_id}`).append(
            `<td class="td-content"><span class="badge badge-soft-danger">${email["password"]}</span></td>`
          );
          exposed_count++;
        }
      }
      $("#emails-count").html(
        `<span class="badge badge-soft-primary">${data["emails"].length}</span>`
      );
      if (exposed_count > 0) {
        $("#exposed_summary").html(
...
function get_employees(scan_id) {
  $.getJSON(
    `/api/queryEmployees/?scan_id=${scan_id}&format=json`,
    function (data) {
      ...
      for (var val in data["employees"]) {
        ...
        $(`#${rand_id}`).append(
          `<td class="td-content">${emp["designation"]}</td>`
        );
      }
      $("#employees-count").html(
        `<span class="badge badge-soft-primary">${data["employees"].length}</span>`
      );
    }
  );
}
...
function get_dorks(scan_id) {
  ...
  $.getJSON(`/api/queryDorks/?scan_id=${scan_id}&format=json`, function (data) {
    ...
    for (var val in data["dorks"]) {
      ...
      $("#dork_type_vertical_tablist").append(
        `<a class="nav-link ${active} mb-1" id="v-${val}-tab" data-bs-toggle="pill" href="#v-${val}" role="tab" aria-controls="v-${val}" aria-selected="true"> ${convertToCamelCase(
          val
        )}</a>`
      );
      // create tab content
...
function get_dork_details(dork_type, scan_id) {
  // render tab modal
  $(".modal-title").html(
    "Dorking Results in category: <b>" + dork_type + "</b>"
  );
  ...
  $.getJSON(
    `/api/queryDorks/?scan_id=${scan_id}&type=${dork_type}&format=json`,
    function (data) {
      $("#modal-loader").empty();
      $("#modal-content").append(
        `<b>${data["dorks"].length} results found in this dork category.</b>`
      );
      ...
      for (dork in data["dorks"]) {
        ...
        $("#dork-detail-modal-ul").append(
          `<li><a href="${dork_obj["url"]}" target="_blank" class="text-primary">${dork_obj["description"]}</a></li>`
        );
      }
    }
  );
...
function get_vulnerability_modal(
  scan_id = null,
  severity = null,
  subdomain_id = null,
  subdomain_name = null
...
function add_todo_for_scanhistory_modal(scan_history_id) {
  ...
  $.getJSON(
    `/api/querySubdomains?scan_id=${scan_history_id}&no_lookup_interesting&format=json`,
    function (data) {
  ...
  fetch("/api/add/recon_note/", {
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        swal.fire(
          "Error!",
          "Could not add recon note, " + response.message,
          "warning",
          {
            button: "Okay",
          }
...
function add_note_for_subdomain(subdomain_id, subdomain_name) {
  ...
  $("#add-todo-subdomain-submit-button").attr(
    "onClick",
    `add_note_for_subdomain_handler(${subdomain_id});`
  );
...
function download_subdomains(
  ...
  if (domain_name) {
    $(".modal-title").html(
      count + " Subdomains for : <b>" + domain_name + "</b>"
    );
  } else {
  ...
  $.getJSON(url, function (data) {
    ...
    if (data["subdomains"].length) {
      ...
      subdomains = "";
      $("#modal-content").append(
        `<textarea class="form-control clipboard copy-txt" id="all_subdomains_text_area" rows="10" spellcheck="false"></textarea>`
      );
      ...
      $("#modal-footer").empty();
      $("#modal-footer").append(
        `<a href="javascript:download('subdomains-${domain_name}.txt', subdomains);" class="m-1 btn btn-dark copyable float-end btn-md"><i class="fe-download me-1"></i> Download Subdomains as txt</a>`
      );
      $("#modal-footer").append(
        `<a href="javascript:;" data-clipboard-action="copy" class="m-1 btn btn-primary copyable float-end btn-md" data-toggle="tooltip" data-placement="top" title="Copy Subdomains!" data-clipboard-target="#all_subdomains_text_area"><i class="fe-copy me-1"></i> Copy Subdomains</a>`
      );
    } else {
...
function download_interesting_subdomains(
  project,
  scan_id = null,
  domain_id = null,
  domain_name = null
  ...
  if (domain_name) {
    $(".modal-title").html(
      count + " Interesting Subdomains for : <b>" + domain_name + "</b>"
    );
  } else {
  ...
  $.getJSON(url, function (data) {
    ...
    if (data.length) {
      ...
      subdomains = "";
      $("#modal-content").append(
        `<textarea class="form-control clipboard copy-txt" id="interesting_subdomains_text_area" rows="10" spellcheck="false"></textarea>`
      );
      ...
      $("#modal-footer").empty();
      $("#modal-footer").append(
        `<a href="javascript:download('interesting_subdomains-${domain_name}.txt', subdomains);" class="m-1 btn btn-dark copyable float-end btn-md"><i class="fe-download me-1"></i> Download Subdomains as txt</a>`
      );
      $("#modal-footer").append(
        `<a href="javascript:;" data-clipboard-action="copy" class="m-1 btn btn-primary copyable float-end btn-md" data-toggle="tooltip" data-placement="top" title="Copy Subdomains!" data-clipboard-target="#interesting_subdomains_text_area"><i class="fe-copy me-1"></i> Copy Subdomains</a>`
      );
    } else {
      swal.fire(
        "No Interesting Subdomains",
        "Could not find any interesting subdomains.",
        "warning",
        {
          button: "Okay",
        }
...
function download_interesting_endpoints(scan_id, domain_name) {
  ...
  if (domain_name) {
    $(".modal-title").html(
      count + " Interesting Endpoints for : <b>" + domain_name + "</b>"
    );
  } else {
  ...
  $.getJSON(url, function (data) {
    ...
    if (data.length) {
      ...
      endpoints = "";
      $("#modal-content").append(
        `<textarea class="form-control clipboard copy-txt" id="interesting_endpoints_text_area" rows="10" spellcheck="false"></textarea>`
      );
      ...
      $("#modal-footer").empty();
      $("#modal-footer").append(
        `<a href="javascript:download('interesting_endpoints-${domain_name}.txt', endpoints);" class="m-1 btn btn-dark copyable float-end btn-md"><i class="fe-download me-1"></i> Download Endpoints as txt</a>`
      );
      $("#modal-footer").append(
        `<a href="javascript:;" data-clipboard-action="copy" class="m-1 btn btn-primary copyable float-end btn-md" data-toggle="tooltip" data-placement="top" title="Copy Endpoints!" data-clipboard-target="#interesting_endpoints_text_area"><i class="fe-copy me-1"></i> Copy Endpoints</a>`
      );
    } else {
      swal.fire(
        "No Interesting Endpoints",
        "Could not find any interesting Endpoints.",
        "warning",
        {
          button: "Okay",
        }
...
function download_important_subdomains(
  scan_id = null,
  domain_id = null,
  domain_name = null
) {
  ...
  if (domain_name) {
    $(".modal-title").html(
      count + " Subdomains marked as important : <b>" + domain_name + "</b>"
    );
  } else {
  ...
  $.getJSON(url, function (data) {
    ...
    if (data["subdomains"].length) {
      ...
      $("#modal-footer").empty();
      $("#modal-footer").append(
        `<a href="javascript:download('important-subdomains-${domain_name}.txt', subdomains);" class="m-1 btn btn-primary copyable float-end btn-md"><i class="fe-download me-1"></i> Download Subdomains as txt</a>`
      );
      $("#modal-footer").append(
        `<a href="javascript:;" data-clipboard-action="copy" class="m-1 btn btn-dark copyable float-end btn-md" data-toggle="tooltip" data-placement="top" title="Copy Subdomains!" data-clipboard-target="#all_subdomains_text_area"><i class="fe-copy me-1"></i> Copy Subdomains</a>`
      );
    } else {
      swal.fire(
        "No Important Endpoints",
        "No subdomains has been marked as important.",
        "warning",
        {
          button: "Okay",
        }
...
function download_endpoints(
  ...
  var url = `/apiwebGuardndpoints/?format=json&only_urls`;

  if (scan_id) {
    url += `&scanwebGuardcan_id}`;
  } else if (domain_id) {
    url += `&target_id=${domain_id}`;
  }
  ...
  if (domain_name) {
    $(".modal-title").html(
      count + " Endpoints for : <b>" + domain_name + "</b>"
    );
  } else {
  ...
  $.getJSON(url, function (data) {
    ...
    endpoints = "";
    $("#modal-content").append(
      `<textarea class="form-control clipboard copy-txt" id="all_endpoints_text_area" rows="10" spellcheck="false"></textarea>`
    );
    ...
    if (domain_name) {
      $("#modal-footer").append(
        `<a href="javascript:download('endpoints-${domain_name}.txt', endpoints);" class="m-1 btn btn-dark copyable float-end btn-md"><i class="fe-download me-1"></i> Download Endpoints as txt</a>`
      );
    } else {
      $("#modal-footer").append(
        `<a href="javascript:download('endpoints-all.txt', endpoints);" class="m-1 btn btn-primary copyable float-end btn-md"><i class="fe-download me-1"></i> Download Endpoints as txt</a>`
      );
    }
    $("#modal-footer").append(
      `<a href="javascript:;" data-clipboard-action="copy" class="m-1 btn btn-primary copyable float-end btn-md" data-toggle="tooltip" data-placement="top" title="Copy Subdomains!" data-clipboard-target="#all_endpoints_text_area"><i class="fe-copy me-1"></i> Copy Endpoints</a>`
    );
  }).fail(function () {});
}
    ...
    fetch("/api/querySubdomains/", {
      ...
      .then(function (response) {
        ...
        if (response["status"]) {
          ...
          subdomains = "";
          $("#modal-content").append(
            `<textarea class="form-control clipboard copy-txt" id="selected_subdomains_text_area" rows="10" spellcheck="false"></textarea>`
          );
          ...
          $("#modal-footer").append(
            `<a href="javascript:;" data-clipboard-action="copy" class="m-1 btn btn-primary copyable float-end btn-md" data-toggle="tooltip" data-placement="top" title="Copy Subdomains!" data-clipboard-target="#selected_subdomains_text_area"><i class="fe-copy me-1"></i> Copy Subdomains</a>`
          );
        } else {
          Swal.fire({
...
function deleteMultipleSubdomains() {
  if (!checkedCount()) {
    ...
    Swal.fire({
      ...
      title:
        "Are you sure you want to delete " + checkedCount() + " Subdomains?",
...
function detect_subdomain_cms(http_url, http_status) {
  if (http_status == 0) {
    var message = `webGuard has earlier identified that this subdomain did not return any HTTP status and likely the subdomain is not alive. webGuard may not be able to detect any CMS, would you still like to continue?`;
  } else if (http_status != 200) {
    var message = `webGuard has earlier identified that this subdomain has HTTP status as ${http_status} and likely that webGuard will not detect any CMS, would you still like to continue?`;
  }
