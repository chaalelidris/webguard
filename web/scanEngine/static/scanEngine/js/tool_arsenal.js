function get_external_tool_latest_version(tool_id, tool_name) {
  var current_version = document.getElementById(
    tool_name + "_current"
  ).textContent;
  console.log(current_version);
  if (
    current_version === "Invalid version lookup command." ||
    current_version === "Version Lookup command not provided."
  ) {
    Swal.fire({
      title: "Unable to fetch latest version!",
      text: `Since the version lookup command is invalid, webGuard is not able to detect if there's a newer version. But you can still force download the latest version.`,
      icon: "info",
    ...
    fetch("/api/github/tool/get_latest_releases/?tool_id=" + tool_id)
      ...
      .then(function (response) {
        ...
        if (response["message"] == "RateLimited") {
          ...
          latest_version =
            latest_version.charAt(0) == "v"
              ? latest_version.substring(1)
              : latest_version;
          ...
          if (
            ...
            Swal.fire({
              title: "Unable to fetch latest version!",
              text: `Since the version lookup command is invalid, webGuard is not able to detect if there's a newer version. But you can still force download the latest version. The latest version is ${latest_version}.`,
              icon: "info",
              confirmButtonText: "Update " + htmlEncode(tool_name),
            }).then((result) => {
              /* Read more about isConfirmed, isDenied below */
            ...
            if (current_version == latest_version) {
              ...
              Swal.fire({
                ...
                text: `Your current version of ${htmlEncode(
                  tool_name
                )} is v${current_version}, but latest version v${latest_version} is available, please update!`,
                icon: "info",
                confirmButtonText: "Update " + htmlEncode(tool_name),
              }).then((result) => {
                if (result.isConfirmed) {
                  ...
                  fetch("/api/tool/update/?tool_id=" + tool_id)
                    ...
                    .then(function (response) {
                      ...
                      Swal.fire({
                        ...
                        text: `${htmlEncode(
                          tool_name
                        )} has now been updated to v${latest_version}!`,
                        icon: "success",
                      });
                    });
                }
...
function get_external_tool_current_version(tool_id, id) {
  fetch("/api/external/tool/get_current_release/?tool_id=" + tool_id)
    ...
    .then(function (response) {
      if (response["status"]) {
        version_number =
          response["version_number"].charAt(0) == "v" ||
          response["version_number"].charAt(0) == "V"
