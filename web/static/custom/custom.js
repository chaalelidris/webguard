...
function get_response_time_text(response_time) {
  if (response_time) {
    ...
    return `<span class="text-${text_color}">${response_time.toFixed(
      4
    )}s</span>`;
  }
  return "";
...
function parse_technology(data, color, scan_id = null, domain_id = null) {
  var badge =
    `<span data-toggle="tooltip" title="Technology" class='badge-link badge badge-soft-` +
    color +
    ` mt-1 me-1'`;
  var data_with_span = "";
  for (var key in data) {
    if (scan_id) {
      data_with_span +=
        badge +
        ` onclick="get_tech_details('${data[key]["name"]}', ${scan_id}, domain_id=null)">` +
        data[key]["name"] +
        "</span>";
    } else if (domain_id) {
      data_with_span +=
        badge +
        ` onclick="get_tech_details('${data[key]["name"]}', scan_id=null, domain_id=domain_id)">` +
        data[key]["name"] +
        "</span>";
    }
  }
...
$("#vulnerability_results").on(
  "click",
  ".btn-delete-vulnerability",
  function () {
...
function report_hackerone(vulnerability_id, severity) {
  ...
  if (severity == "Info" || severity == "Low" || severity == "Medium") {
    message =
      "We do not recommended sending this vulnerability report to hackerone due to the severity, do you still want to report this?";
  } else {
    message = "This vulnerability report will be sent to Hackerone.";
  }
  const vulnerability_report_api =
    "/api/vulnerability/report/?vulnerability_id=" + vulnerability_id;
  swal.queue([
    {
        ...
        return fetch(vulnerability_report_api, {
          ...
          .catch(function () {
            swal.insertQueueStep({
              icon: "error",
              title:
                "Oops! Unable to send vulnerability report to hackerone, check your target team_handle or hackerone configurarions!",
            });
          });
      },
    },
  ...
  var interesting_subdomain_table = $("#interesting_subdomains").DataTable({
    drawCallback: function (settings, start, end, max, total, pre) {
      ...
      if (this.fnSettings().fnRecordsTotal() == 0) {
        ...
        // <div class="card-body">
        // webGuard could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
        // </div>
        // </div>`);
      } else {
        // show nav bar
        $(".interesting-tab-show").removeAttr("style");
        $("#interesting_subdomain_alert_count").html(
          `${this.fnSettings().fnRecordsTotal()} Interesting Subdomains`
        );
        $("#interesting_subdomain_count_badge").empty();
        $("#interesting_subdomain_count_badge").html(
          `<span class="badge badge-soft-primary me-1">${this.fnSettings().fnRecordsTotal()}</span>`
        );
      }
    },
    oLanguage: {
    ...
    processing: true,
    dom:
      "<'dt--top-section'<'row'<'col-12 col-sm-6 d-flex justify-content-sm-start justify-content-center'f><'col-12 col-sm-6 d-flex justify-content-sm-end justify-content-center'l>>>" +
      "<'table-responsive'tr>" +
      ...
      },
      {
        render: function (data, type, row) {
          ...
          if (data >= 200 && data < 300) {
            return (
              "<span class='badge badge-pills badge-soft-success'>" +
              data +
              "</span>"
            );
          } else if (data >= 300 && data < 400) {
            return (
              "<span class='badge badge-pills badge-soft-warning'>" +
              data +
              "</span>"
            );
          } else if (data == 0) {
          ...
          return (
            `<span class='badge badge-pills badge-soft-danger'>` +
            data +
            `</span>`
          );
        },
  ...
  $("#interesting_endpoints").DataTable({
    drawCallback: function (settings, start, end, max, total, pre) {
      if (this.fnSettings().fnRecordsTotal() == 0) {
        ...
        $(".interesting-tab-show").removeAttr("style");
        $("#interesting_endpoint_alert_count").html(
          `, ${this.fnSettings().fnRecordsTotal()} Interesting Endpoints`
        );
        $("#interesting_endpoint_count_badge").empty();
        $("#interesting_endpoint_count_badge").html(
          `<span class="badge badge-soft-primary me-1">${this.fnSettings().fnRecordsTotal()}</span>`
        );
      }
    },
...
function get_important_subdomains(target_id, scan_history_id) {
  ...
  $.getJSON(url, function (data) {
    ...
    if (data["subdomains"].length > 0) {
      $("#important-count").html(
        `<span class="badge badge-soft-primary ms-1 me-1">${data["subdomains"].length}</span>`
      );
      ...
      $("#important-count").html(
        `<span class="badge badge-soft-primary ms-1 me-1">0</span>`
      );
      $("#important-subdomains-list").append(
        `<p>No subdomains marked as important!</p>`
      );
    }
    $(".bs-tooltip").tooltip();
  });
...
function mark_important_subdomain(row, subdomain_id) {
  ...
  if ($("#important_subdomain_" + subdomain_id).length == 0) {
    $("#subdomain-" + subdomain_id).prepend(
      `<span id="important_subdomain_${subdomain_id}"></span>`
    );
...
function delete_datatable_rows(
  table_id,
  rows_id,
  show_snackbar = true,
  snackbar_title
  ...
  for (var row in rows_id) {
    table
      .row(table_id + "_row_" + rows_id[row])
      .remove()
      .draw();
  }
  Snackbar.show({
...
function show_subscan_results(subscan_id) {
  ...
  var api_url =
    "/api/fetch/results/subscan/?format=json&subscan_id=" + subscan_id;
  Swal.fire({
  ...
  fetch(api_url, {
    ...
    .then(function (response) {
      ...
      if (response["subscan"]["status"] == -1) {
        swal.fire(
          "Error!",
          "Scan has not yet started! Please wait for other scans to complete...",
          "warning",
          {
            button: "Okay",
          }
      ...
      $("#xl-modal_title").html(
        `${task_name} Results on ${response["subscan"]["subdomain_name"]}`
      );
      var scan_status = "";
      ...
      $("#xl-modal-content").append(
        `<div>Scan Status: <span class="badge bg-${badge_color}">${scan_status}</span></div>`
      );
      $("#xl-modal-content").append(
        `<div class="mt-1">Engine Used: <span class="badge bg-primary">${htmlEncode(
          response["subscan"]["engine"]
        )}</span></div>`
      );
        ...
        if (response["subscan"]["task"] == "port_scan") {
          ...
          for (var ip in response["result"]) {
            ...
            $("#port_results_li").append(
              `<h5>IP Address: ${ip_addr}</br></br>${response["result"][ip]["ports"].length} Ports Open</h5>`
            );
            ...
            for (var port_obj in response["result"][ip]["ports"]) {
              ...
              $(`#${id_name}`).append(
                `<li><span class="ms-1 mt-1 me-1 badge badge-soft-${port_color}">${port["number"]}</span>/<span class="ms-1 mt-1 me-1 badge badge-soft-${port_color}">${port["service_name"]}</span>/<span class="ms-1 mt-1 me-1 badge badge-soft-${port_color}">${port["description"]}</span></li>`
              );
            }
          }
          $("#xl-modal-footer").append(
            `<span class="text-danger">* Uncommon Ports</span>`
          );
        } else if (response["subscan"]["task"] == "vulnerability_scan") {
          render_vulnerability_in_xl_modal(
            (vuln_count = response["result"].length),
            (subdomain_name = response["subscan"]["subdomain_name"]),
            (result = response["result"])
          );
        } else if (response["subscan"]["task"] == "fetch_url") {
          render_endpoint_in_xlmodal(
            (endpoint_count = response["result"].length),
            (subdomain_name = response["subscan"]["subdomain_name"]),
            (result = response["result"])
          );
        } else if (response["subscan"]["task"] == "dir_file_fuzz") {
          if (response["result"][0]["directory_files"].length == 0) {
            ...
            render_directories_in_xl_modal(
              response["result"][0]["directory_files"].length,
              response["subscan"]["subdomain_name"],
              response["result"][0]["directory_files"]
            );
          }
        }
      } else {
...
function render_endpoint_in_xlmodal(endpoint_count, subdomain_name, result) {
  ...
  $("#xl-modal-content").append(
    `<h5> ${endpoint_count} Endpoints Discovered on subdomain ${subdomain_name}</h5>`
  );
  $("#xl-modal-content").append(`
		<div class="">
  ...
  for (var endpoint_obj in result) {
    ...
    $("#endpoint_tbody").append(`
			...
			<td>${parse_comma_values_into_span(
...
function render_vulnerability_in_xl_modal(vuln_count, subdomain_name, result) {
  ...
  $("#xl-modal-content").append(
    `<h5> ${vuln_count} Vulnerabilities Discovered on subdomain ${subdomain_name}</h5>`
  );
  $("#xl-modal-content").append(
    `<ol id="vuln_results_ol" class="list-group list-group-numbered"></ol>`
  );
  $("#xl-modal-content").append(`
		<div class="">
  ...
  for (var vuln in result) {
    ...
    var vuln_type = vuln_obj["type"]
      ? `<span class="badge badge-soft-primary">&nbsp;&nbsp;${vuln_obj[
          "type"
        ].toUpperCase()}&nbsp;&nbsp;</span>`
      : "";
    var tags = "";
    ...
    if (vuln_obj["cvss_score"]) {
      if (vuln_obj["cvss_score"] > 0.1 && vuln_obj["cvss_score"] <= 3.9) {
        ...
        vuln_obj["cvss_score"] > 3.9 &&
        vuln_obj["cvss_score"] <= 6.9
      ) {
        badge = "warning";
      } else if (
        vuln_obj["cvss_score"] > 6.9 &&
        vuln_obj["cvss_score"] <= 8.9
      ) {
        badge = "danger";
      }
    ...
    cve_cwe_badge += "</div>";
    var http_url = vuln_obj["http_url"].includes("http")
      ? "<a href='" +
        htmlEncode(vuln_obj["http_url"]) +
...
function render_directories_in_xl_modal(
  directory_count,
  subdomain_name,
  result
) {
  $("#xl-modal-content").append(
    `<h5> ${directory_count} Directories Discovered on subdomain ${subdomain_name}</h5>`
  );
  $("#xl-modal-content").append(`
		<div class="">
...
function get_and_render_subscan_history(subdomain_id, subdomain_name) {
  ...
  fetch("/api/listSubScans/?format=json", {
    ...
    .then(function (data) {
      ...
      if (data["status"]) {
        $("#modal_title").html(
          "Subscan History for subdomain " + subdomain_name
        );
        $("#modal-content").empty();
        ...
        for (var result in data["results"]) {
          ...
          if (result_obj.status == 0) {
            ...
            bg_color = "bg-soft-danger";
            status_badge =
              '<span class="float-end badge bg-danger">Aborted</span>';
          } else if (result_obj.status == 2) {
            color = "success";
            bg_color = "bg-soft-success";
            status_badge =
              '<span class="float-end badge bg-success">Task Completed</span>';
          }

          $("#subscan_history_table").append(`
					<div class="card border-${color} border mini-card">
					<a href="#" class="text-reset item-hovered" onclick="show_subscan_results(${
            result_obj["id"]
          })">
					<div class="card-header ${bg_color} text-${color} mini-card-header">
					${task_name} on <b>${
            result_obj.subdomain_name
          }</b> using engine <b>${htmlEncode(result_obj.engine)}</b>
					</div>
...
function fetch_whois(domain_name, force_reload_whois = false) {
  ...
  fetch(url, {})
    ...
    .then(function (response) {
      ...
      document.getElementById("domain_age").innerHTML =
        response["domain"]["domain_age"] +
        " " +
        response["domain"]["date_created"];
      document.getElementById("ip_address").innerHTML =
        response["domain"]["ip_address"];
      document.getElementById("ip_geolocation").innerHTML =
        response["domain"]["geolocation"];

      document.getElementById("registrant_name").innerHTML =
        response["registrant"]["name"];
      console.log(response["registrant"]["organization"]);
      document.getElementById("registrant_organization").innerHTML = response[
        "registrant"
      ]["organization"]
        ? response["registrant"]["organization"]
        : " ";
      document.getElementById("registrant_address").innerHTML =
        response["registrant"]["address"] +
        " " +
        response["registrant"]["city"] +
        " " +
        response["registrant"]["state"] +
        " " +
        response["registrant"]["country"];
      document.getElementById("registrant_phone_numbers").innerHTML =
        response["registrant"]["tel"];
      document.getElementById("registrant_fax").innerHTML =
        response["registrant"]["fax"];
      ...
      if (data["status"]) {
        ...
        for (var result in data["results"]) {
          ...
          if (result_obj.status == 0) {
            ...
            status_badge =
              '<span class="float-end badge bg-primary">Running</span>';
          }
...
function get_ips(scan_id = null, domain_id = null) {
  ...
  $.getJSON(url, function (data) {
    ...
    for (var val in data["ips"]) {
      ...
      if (scan_id) {
        $("#ip-address").append(
          `<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${ip["ports"].length} Ports Open." onclick="get_ip_details('${ip["address"]}', scan_id=${scan_id}, domain_id=null)">${ip["address"]}</span>`
        );
      } else if (domain_id) {
        $("#ip-address").append(
          `<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${ip["ports"].length} Ports Open." onclick="get_ip_details('${ip["address"]}', scan_id=null, domain_id=${domain_id})">${ip["address"]}</span>`
        );
      }
      // $("#ip-address").append(`<span class='badge badge-soft-${badge_color}  m-1' data-toggle="modal" data-target="#tabsModal">${ip['address']}</span>`);
    }
    $("#ip-address-count").html(
      `<span class="badge badge-soft-primary me-1">${data["ips"].length}</span>`
    );
...
function get_technologies(scan_id = null, domain_id = null) {
  ...
  $.getJSON(url, function (data) {
    ...
    for (var val in data["technologies"]) {
      ...
      if (scan_id) {
        $("#technologies").append(
          `<span class='badge badge-soft-primary  m-1 badge-link' data-toggle="tooltip" title="${tech["count"]} Subdomains use this technology." onclick="get_tech_details('${tech["name"]}', scan_id=${scan_id}, domain_id=null)">${tech["name"]}</span>`
        );
      } else if (domain_id) {
        $("#technologies").append(
          `<span class='badge badge-soft-primary  m-1 badge-link' data-toggle="tooltip" title="${tech["count"]} Subdomains use this technology." onclick="get_tech_details('${tech["name"]}', scan_id=null, domain_id=${domain_id})">${tech["name"]}</span>`
        );
      }
    }
    $("#technologies-count").html(
      `<span class="badge badge-soft-primary me-1">${data["technologies"].length}</span>`
    );
    $("body").tooltip({ selector: "[data-toggle=tooltip]" });
  });
}
...
function get_ports(scan_id = null, domain_id = null) {
  ...
  $.getJSON(url, function (data) {
    ...
    for (var val in data["ports"]) {
      ...
      if (scan_id) {
        $("#ports").append(
          `<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${port["description"]}" onclick="get_port_details('${port["number"]}', scan_id=${scan_id}, domain_id=null)">${port["number"]}/${port["service_name"]}</span>`
        );
      } else if (domain_id) {
        $("#ports").append(
          `<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${port["description"]}" onclick="get_port_details('${port["number"]}', scan_id=null, domain_id=${domain_id})">${port["number"]}/${port["service_name"]}</span>`
        );
      }
    }
    $("#ports-count").html(
      `<span class="badge badge-soft-primary me-1">${data["ports"].length}</span>`
    );
    $("body").tooltip({ selector: "[data-toggle=tooltip]" });
  });
}
...
function get_ip_details(ip_address, scan_id = null, domain_id = null) {
  ...
  $("#modal-content").append(
    `<ul class='nav nav-tabs nav-bordered' id="modal_tab_nav"></ul><div id="modal_tab_content" class="tab-content"></div>`
  );

  $("#modal_tab_nav").append(
    `<li class="nav-item"><a class="nav-link active" data-bs-toggle="tab" href="#modal_content_port" aria-expanded="true"><span id="modal-open-ports-count"></span>Open Ports &nbsp;${port_loader}</a></li>`
  );
  $("#modal_tab_nav").append(
    `<li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#modal_content_subdomain" aria-expanded="false"><span id="modal-subdomain-count"></span>Subdomains &nbsp;${subdomain_loader}</a></li>`
  );

  // add content area
  $("#modal_tab_content").empty();
  $("#modal_tab_content").append(
    `<div class="tab-pane show active" id="modal_content_port"></div><div class="tab-pane" id="modal_content_subdomain"></div>`
  );

  $("#modal-open-ports").append(
    `<div class="modal-text" id="modal-text-open-port"></div>`
  );
  ...
  $.getJSON(port_url, function (data) {
    ...
    $("#modal_content_port").append(
      `<p> IP Addresses ${ip_address} has ${data["ports"].length} Open Ports`
    );
    $("#modal-open-ports-count").html(
      `<b>${data["ports"].length}</b>&nbsp;&nbsp;`
    );
    for (port in data["ports"]) {
      port_obj = data["ports"][port];
      badge_color = port_obj["is_uncommon"] ? "danger" : "info";
      $("#modal_content_port").append(
        `<li class="mt-1">${port_obj["description"]} <b class="text-${badge_color}">(${port_obj["number"]}/${port_obj["service_name"]})</b></li>`
      );
    }
  ...
  $.getJSON(subdomain_url, function (data) {
    ...
    $("#modal_content_subdomain").append(
      `<p>${data["subdomains"].length} Subdomains are associated with IP ${ip_address}`
    );
    $("#modal-subdomain-count").html(
      `<b>${data["subdomains"].length}</b>&nbsp;&nbsp;`
    );
    for (subdomain in data["subdomains"]) {
      ...
      if (subdomain_obj["http_url"]) {
        $("#modal_content_subdomain").append(
          `<li class="mt-1" id="${li_id}"><a href='${subdomain_obj["http_url"]}' target="_blank" class="text-${badge_color}">${subdomain_obj["name"]}</a></li>`
        );
      } else {
        $("#modal_content_subdomain").append(
          `<li class="mt-1 text-${badge_color}" id="${li_id}">${subdomain_obj["name"]}</li>`
        );
      }
    ...
    $("#modal-text-subdomain").append(
      `<span class="float-end text-danger">*Subdomains highlighted are 40X HTTP Status</span>`
    );
...
function get_port_details(port, scan_id = null, domain_id = null) {
  ...
  $("#modal_tab_nav").append(
    `<li class="nav-item"><a class="nav-link active" data-bs-toggle="tab" href="#modal_content_ip" aria-expanded="true"><span id="modal-ip-count"></span>IP Address&nbsp;${ip_spinner}</a></li>`
  );
  $("#modal_tab_nav").append(
    `<li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#modal_content_subdomain" aria-expanded="false"><span id="modal-subdomain-count"></span>Subdomains&nbsp;${subdomain_spinner}</a></li>`
  );

  // add content area
  $("#modal_tab_content").append(
    `<div class="tab-pane show active" id="modal_content_ip"></div><div class="tab-pane" id="modal_content_subdomain"></div>`
  );
  ...
  $.getJSON(ip_url, function (data) {
    $("#modal_ip_ul").empty();
    $("#modal_ip_ul").append(
      `<p>${data["ips"].length} IP Addresses have Port ${port} Open`
    );
    ...
    for (ip in data["ips"]) {
      ...
      $("#modal_ip_ul").append(
        `<li class='mt-1 text-${text_color}'>${ip_obj["address"]}</li>`
      );
    }
    $("#modal_ip_ul").append(
      `<span class="float-end text-warning">*IP Address highlighted are CDN IP Address</span>`
    );
  ...
  $.getJSON(subdomain_url, function (data) {
    ...
    $("#modal_subdomain_ul").append(
      `<p>${data["subdomains"].length} Subdomains have Port ${port} Open`
    );
    $("#modal-subdomain-count").html(
      `<b>${data["subdomains"].length}</b>&nbsp;&nbsp;`
    );
    for (subdomain in data["subdomains"]) {
      ...
      if (subdomain_obj["http_url"]) {
        $("#modal_subdomain_ul").append(
          `<li id="${li_id}" class="mt-1"><a href='${subdomain_obj["http_url"]}' target="_blank" class="text-${badge_color}">${subdomain_obj["name"]}</a></li>`
        );
      } else {
        $("#modal_subdomain_ul").append(
          `<li class="mt-1 text-${badge_color}" id="${li_id}">${subdomain_obj["name"]}</li>`
...
function get_tech_details(tech, scan_id = null, domain_id = null) {
  ...
  $("#modal-footer").empty();
  $(".modal-text").append(
    `<div class='outer-div' id="modal-loader"><span class="inner-div spinner-border text-primary align-self-center loader-sm"></span></div>`
  );
  // query subdomains
  $.getJSON(url, function (data) {
    $("#modal-loader").empty();
    $("#modal-content").empty();
    $("#modal-content").append(
      `${data["subdomains"].length} Subdomains are using ${tech}`
    );
    for (subdomain in data["subdomains"]) {
      subdomain_obj = data["subdomains"][subdomain];
      badge_color = subdomain_obj["http_status"] >= 400 ? "danger" : "";
      ...
      if (subdomain_obj["http_url"]) {
        $("#modal-content").append(
          `<li id="${li_id}"><a href='${subdomain_obj["http_url"]}' target="_blank" class="text-${badge_color}">${subdomain_obj["name"]}</a></li>`
        );
      } else {
        $("#modal-content").append(
          `<li class="text-${badge_color}" id="${li_id}">${subdomain_obj["name"]}</li>`
        );
      }
      ...
      if (response.status) {
        ...
        content += `
				<div class="tab-pane fade active show" id="v-pills-cve-details" role="tabpanel" aria-labelledby="v-pills-cve-details-tab" data-simplebar style="max-height: 600px; min-height: 600px;">
					...
					<span class="badge badge-soft-primary">Assigner: ${
            response.result.assigner
          }</span>
					<span class="badge badge-outline-primary">CVSS Vector: ${
            response.result["cvss-vector"]
          }</span>
					<table class="domain_details_table table table-hover table-borderless">
						...
						<tr>
							<td>CVSS Score</td>
							<td><span class="badge badge-soft-${cvss_score_badge}">${
          response.result.cvss ? response.result.cvss : "-"
        }</span></td>
						</tr>
						<tr>
                ...
                response.result.impact.confidentiality
                  ...
                  : "N/A"
						...
						<tr>
							...
							<td>${
                response.result.impact.integrity
                  ? response.result.impact.integrity
                  : "N/A"
              }</td>
						</tr>
						<tr>
							...
							<td>${
                response.result.impact.availability
                  ? response.result.impact.availability
                  : "N/A"
              }</td>
						</tr>
						<tr>
							...
							<td>${
                response.result.access.complexity
                  ? response.result.access.complexity
                  : "N/A"
              }</td>
						</tr>
						<tr>
							...
							<td>${
                response.result.access.authentication
                  ? response.result.access.authentication
                  : "N/A"
              }</td>
						</tr>
						<tr>
							<td>CWE ID</td>
							<td><span class="badge badge-outline-danger">${
                response.result.cwe ? response.result.cwe : "N/A"
              }</span></td>
						</tr>
					</table>
				</div>
...
function get_most_vulnerable_target(
  slug = null,
  scan_id = null,
  target_id = null,
  ignore_info = false,
  ...
  $("#most_vulnerable_spinner").append(
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        $("#most_vulnerable_target_div").append(`
				...
				Once the vulnerability scan is performed, webGuard will identify the most vulnerable targets.</div>
			`);
      }
    });
}

function get_most_common_vulnerability(
  slug = null,
  scan_id = null,
  target_id = null,
  ignore_info = false,
  ...
  $("#most_common_vuln_spinner").append(
    `<div class="spinner-border text-primary m-2" role="status"></div>`
  );
  var data = {};
  if (scan_id) {
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        $("#most_common_vuln_div").append(`
				...
				Once the vulnerability scan is performed, webGuard will identify the Most Common Vulnerabilities.</div>
			`);
      }
    });
}
...
function render_vuln_offcanvas(vuln) {
  ...
  body += `<p><b>Severity: </b>${
    vuln.severity
  }<br><b>Type: </b>${vuln.type.toUpperCase()}<br><b>Source: </b> ${vuln.source.toUpperCase()}</p>`;
  ...
  if (vuln.references.length) {
    ...
    vuln.references.forEach((reference) => {
      body += `<li><a href="${htmlEncode(
        reference.url
      )}" target="_blank">${htmlEncode(reference.url)}</a></li>`;
    });
...
async function fetch_gpt_vuln_details(id, title) {
  ...
  var text =
    "Please wait while the GPT is generating vulnerability description.";
  try {
...
async function show_attack_surface_modal(id) {
  ...
  try {
    ...
    if (data.status) {
      $("#modal_title").html(
        `Attack Surface Suggestion for ${data.subdomain_name} (BETA)`
      );
      $("#modal-content").empty();
      $("#modal-content").append(
        data.description.replace(new RegExp("\r?\n", "g"), "<br />")
      );
      $("#modal_dialog").modal("show");
    } else {
      Swal.close();
...
function show_scan_configuration(
  starting_path,
		...
		<ul class="nav nav-tabs nav-bordered" role="tablist">
			<li class="nav-item" role="presentation">
				<a href="#outofscope" data-bs-toggle="tab" aria-expanded="true" class="nav-link active" role="tab">
					Out of Scope Subdomains <span class="badge bg-secondary">${
            out_of_scope_subdomains.length
          }</span>
				</a>
			</li>
			<li class="nav-item" role="presentation">
				<a href="#imported" data-bs-toggle="tab" aria-expanded="false" class="nav-link" role="tab">
					Imported Subdomains <span class="badge bg-secondary">${
            imported_subdomains.length
          }</span>
				</a>
			</li>
			<li class="nav-item" role="presentation">
		...
		<div class="tab-content">
			<div class="tab-pane fade show active" id="outofscope" role="tabpanel">
				<ul class="config-list">
					${out_of_scope_subdomains
            .map((domain) => `<li><code>${domain}</code></li>`)
            .join("")}
				</ul>
			</div>
...
function handleSyncBookmarkedProgramsSwal() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      const currentProjectSlug = document.body.getAttribute(
        "data-current-project"
      );
      fetch(
        `/api/hackerone-programs/sync_bookmarked?project_slug=${currentProjectSlug}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
